<?php
session_start();
class ToupiaoxiangmuController extends CommonController {
    ...
    public function page(){
			...
			if(in_array($k, $this->columData)){
                if ($val != ''){
                    $where.= " and ".$k." like '".$val."'";
                                        ...
                                        $sql = "select * from `toupiaoxiangmu` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $startCount = ($page-1)*$limt;
                    $lists = "select * from `toupiaoxiangmu` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
                ...
                $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `toupiaoxiangmu` (".$k.") VALUES (".$v.")";
    ...
    public function update(){
        ...
        $value = implode(',',$v);
         $sql = "UPDATE toupiaoxiangmu SET ".$value." where id = ".$id;
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function delete(){
        ...
        $sql = "delete from toupiaoxiangmu WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                ...
                $this->andone('toupiaoxiangmu',$id,$column='clicknum',$num=1);
                        $clicktime = "update toupiaoxiangmu set clicktime = ".date('Y-m-d H:i:s',time())." where ".$where;
        table_sql($clicktime);
                $sql = "select * from `toupiaoxiangmu` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function thumbsup($id=false){
        ...
        if ($type==1){
            $result = $this->andone('toupiaoxiangmu',$id,$column = 'thumbsupnum',$num = 1);
        }else{
            $result = $this->andone('toupiaoxiangmu',$id,$column = 'crazilynum',$num = 1);
        }
		...
		$limt = isset($_REQUEST['limit'])?$_REQUEST['limit']:"5";
				$sort = 'clicknum';
				$lists = "select * from `toupiaoxiangmu` order by ".$sort." desc limit 0,".$limt;
		$result = table_sql($lists);
		$arrayData = array();
		if ($result->num_rows > 0) {
			while ($datas = $result->fetch_assoc()){
    ...
    public function remind($columnName,$type){
        ...
        $sql = "select * from `toupiaoxiangmu` where 1 ".$where;
        $result = table_sql($sql);
